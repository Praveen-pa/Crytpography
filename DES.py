...
# --- S-DES Encrypt / Decrypt one block ---
...
# --- CBC Mode ---
def cbc_encrypt(plaintext, key10, iv):
    K1, K2 = generate_keys(key10)
    blocks = [plaintext[i:i+8] for i in range(0,len(plaintext),8)]
    prev = [int(b) for b in iv]
    ciphertext = []
    for block in blocks:
        pt_bits = [int(b) for b in block]
        xored = xor(pt_bits, prev)
        ct_bits = sdes_encrypt_block(xored, K1, K2)
        ciphertext.extend(ct_bits)
        prev = ct_bits
    return ''.join(str(b) for b in ciphertext)

def cbc_decrypt(ciphertext, key10, iv):
    K1, K2 = generate_keys(key10)
    blocks = [ciphertext[i:i+8] for i in range(0,len(ciphertext),8)]
    prev = [int(b) for b in iv]
    plaintext = []
    for block in blocks:
        ct_bits = [int(b) for b in block]
        temp = sdes_decrypt_block(ct_bits, K1, K2)
        pt_bits = xor(temp, prev)
        plaintext.extend(pt_bits)
        prev = ct_bits
    return ''.join(str(b) for b in plaintext)
