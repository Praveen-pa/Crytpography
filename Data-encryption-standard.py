# des.py
# Educational implementation of DES encryption/decryption in Python
...
def des_key_schedule(key64):
    key56 = permute(key64, PC1)
    C, D = key56[:28], key56[28:]
    subkeys = []
    for i in range(16):
        C = left_rotate(C, SHIFT[i])
        D = left_rotate(D, SHIFT[i])
...
def str_to_bitlist(data):
    return [int(x) for c in data for x in f"{ord(c):08b}"]
...
if __name__ == "__main__":
    # Example
    key = "abcdefgh"   # 8-byte key (64 bits with parity bits)
    plaintext = "DESdemo!"  # 8-byte block
    
    key_bits = str_to_bitlist(key)
    pt_bits = str_to_bitlist(plaintext)
    
    subkeys = des_key_schedule(key_bits)
    ct_bits = des_encrypt_block(pt_bits, subkeys)
    decrypted_bits = des_decrypt_block(ct_bits, subkeys)
    
    print("Plaintext:", plaintext)
    print("Ciphertext (hex):", hex(int(''.join(map(str, ct_bits)), 2)))
